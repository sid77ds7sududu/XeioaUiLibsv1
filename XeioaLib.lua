...
if syn then
        ...
        Xeioa.Parent = gethui() or game.CoreGui
end

if gethui then
        for _, Interface in ipairs(gethui():GetChildren()) do
                if Interface.Name == Xeioa.Name and Interface ~= Xeioa then
                        Interface:Destroy()
                end
        end
else
...
function XeioaLib:IsRunning()
        if gethui then
                return Xeioa.Parent == gethui()
        else
                return Xeioa.Parent == game:GetService("CoreGui")
        end
end
...
local function AddThemeObject(Object, Type)
        if not XeioaLib.ThemeObjects[Type] then
                XeioaLib.ThemeObjects[Type] = {}
        end
        table.insert(XeioaLib.ThemeObjects[Type], Object)
        Object[ReturnProperty(Object)] = XeioaLib.Themes[XeioaLib.SelectedTheme][Type]
        return Object
end

local function SetTheme()
        for Name, Type in pairs(XeioaLib.ThemeObjects) do
                for _, Object in pairs(Type) do
                        Object[ReturnProperty(Object)] = XeioaLib.Themes[XeioaLib.SelectedTheme][Name]
                end
        end
end
...
local function LoadCfg(Config)
        ...
        table.foreach(Data, function(a, b)
                if XeioaLib.Flags[a] then
                        spawn(function()
                                if XeioaLib.Flags[a].Type == "Colorpicker" then
                                        XeioaLib.Flags[a]:Set(UnpackColor(b))
                                else
                                        XeioaLib.Flags[a]:Set(b)
                                end
                        end)
                end
        end)
...
local function SaveCfg(Name)
        local Data = {}
        for i, v in pairs(XeioaLib.Flags) do
                if v.Save then
        ...
        writefile(XeioaLib.Folder .. "/" .. Name .. ".txt", tostring(HttpService:JSONEncode(Data)))
end
...
function XeioaLib:MakeNotification(NotificationConfig)
        spawn(function()
                ...
                local NotificationFrame = SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(10, 10, 10), 0, 10), {
                        ...
                        MakeElement("Stroke", Color3.fromRGB(70, 70, 70), 1.2),
...
function XeioaLib:Init()
        if XeioaLib.SaveCfg then
                pcall(function()
...
function XeioaLib:MakeWindow(WindowConfig)
        ...
        WindowConfig.IntroIcon = WindowConfig.IntroIcon or "rbxassetid://93244832198617"
        XeioaLib.Folder = WindowConfig.ConfigFolder
        XeioaLib.SaveCfg = WindowConfig.SaveConfig
        ...
        AddDraggingFunctionality(OpenButton, OpenButton)

        AddConnection(OpenButton.MouseEnter, function()
                TweenService:Create(OpenButton, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {BackgroundColor3 = Color3.fromRGB(35, 35, 35)}):Play()
        end)
        AddConnection(OpenButton.MouseLeave, function()
                TweenService:Create(OpenButton, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {BackgroundColor3 = Color3.fromRGB(18, 18, 18)}):Play()
        end)
        ...
        local function ShowOpenButton()
                ...
                TweenService:Create(OpenButton, TweenInfo.new(0.3, Enum.EasingStyle.Quint), {BackgroundTransparency = 0, TextTransparency = 0}):Play()
        end

        local function HideOpenButton()
                TweenService:Create(OpenButton, TweenInfo.new(0.25, Enum.EasingStyle.Quint), {BackgroundTransparency = 1, TextTransparency = 1}):Play()
                task.delay(0.3, function() OpenButton.Visible = false end)
        end
        ...
        function TabFunction:MakeTab(TabConfig)
                ...
                local function GetElements(ItemParent)
                        ...
                        function ElementFunction:AddToggle(ToggleConfig)
                                ...
                                local TrackOff = Color3.fromRGB(45, 45, 45)
                                local TrackOn  = Color3.fromRGB(230, 230, 230)

                                local ToggleTrack = Create("Frame", {
                                        Size = UDim2.new(0, 40, 0, 22),
                                        AnchorPoint = Vector2.new(1, 0.5),
                                        Position = UDim2.new(1, -14, 0.5, 0),
                                        BackgroundColor3 = TrackOff,
                                        BorderSizePixel = 0,
                                        ClipsDescendants = true
                                }, {
                                ...
                                function Toggle:Set(Value)
                                        Toggle.Value = Value
                                        local knobTarget = Toggle.Value and UDim2.new(1, -19, 0.5, 0) or UDim2.new(0, 3, 0.5, 0)
                                        local trackColor = Toggle.Value and TrackOn or TrackOff
                                        local knobColor  = Toggle.Value and Color3.fromRGB(10, 10, 10) or Color3.fromRGB(160, 160, 160)
                                        local strokeColor = Toggle.Value and Color3.fromRGB(200, 200, 200) or Color3.fromRGB(70, 70, 70)
                                        TweenService:Create(ToggleKnob, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Position = knobTarget, BackgroundColor3 = knobColor}):Play()
                                        TweenService:Create(ToggleTrack, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {BackgroundColor3 = trackColor}):Play()
                                        TweenService:Create(ToggleTrack.TrackStroke, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Color = strokeColor}):Play()
